{-|
Module Aetherling.Simulator.State
Description: Internal State data used by the simulator.
-}
module Aetherling.Simulator.State (
  Simhl,
  SimhlPre,
...
--
-- simhlPreResult function does things 1 and 2.
...
-- | Function that encapsulates the typical acts a preprocessor pass
-- for an op needs to do before producing its return value: update the
-- maximum stream length field, and append warnings (along with a
-- formatted op stack) if needed to the SimhlPreState.
--
-- Arguments:
--
...
--
-- This is meant for helping the user figure out exactly where the Op that
-- generated an error message in in the DAG.
...
--
-- It's recommended that simhlPreResult is used instead of this function.
